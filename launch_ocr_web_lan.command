...
if [ -n "$LAN_IP" ]; then
  /usr/bin/osascript -e "display notification \"LAN: http://${LAN_IP}:${TARGET_PORT}\" with title \"OCR Web 已启动\" subtitle \"本机: http://127.0.0.1:${TARGET_PORT}\"" || true
fi

export OCR_WEB_HOST="0.0.0.0"
